...
export function SearchableSelect({ label, placeholder, options, value, onChange, isAr, triggerClassName, containerClassName, valueClassName }: SearchableSelectProps) {
  ...
  return (
    <div className={cn("space-y-1.5 w-full relative", containerClassName)} ref={dropdownRef}>
      ...
      >
        <span className={cn("truncate", !value && "text-slate-400 font-normal", valueClassName)}>
          {value || placeholder}
        </span>
            ...
            <div className="p-1 flex-1">
              {filteredOptions.length > 0 ? filteredOptions.map(opt => (
                <div 
                  ...
                  className={cn(
                    "px-3 py-2 text-xs font-bold cursor-pointer rounded-lg transition-colors flex items-center justify-between group",
                    value === opt ? "bg-orange-500 text-white" : "text-slate-700 hover:bg-orange-50"
                  )}
                >
