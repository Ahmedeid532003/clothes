...
import { SupplierDiscountNoteContainer } from './SupplierDiscountNoteContainer';
...
interface SupplierDiscountNoteTabProps {
  lang: "ar" | "en";
...
export function SupplierDiscountNoteTab({ lang, suppliers: compositeItems, setSuppliers: setCompositeItems, products, groups = [] }: SupplierDiscountNoteTabProps) {
        ...
        <div className="flex items-center justify-between h-full gap-4">
          <div className="space-y-0.5 overflow-hidden">
            <h2 className="text-base sm:text-lg font-bold text-slate-800 flex items-center gap-2 truncate">
              ...
              <span className="truncate">{isAr ? "إشعار خصم من مورد" : "Discount Note from Supplier"}</span>
            </h2>
            <p className="text-[10px] sm:text-xs text-slate-500 truncate">
              {isAr ? "إدارة إشعار خصم من مورد والتعاملات الخاصة بهم" : "Manage discount notes from suppliers"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span className="text-[11px] whitespace-nowrap font-black hidden sm:inline-block">
                {isAr ? "اضافه اشعار خصم جديد" : "Create New"}
              </span>
            </button>
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black tracking-tight leading-tight">
                        {editingId 
                          ? (isAr ? "تعديل إشعار خصم من مورد" : "Edit Discount Note from Supplier")
                          : (isAr ? "اضافه اشعار خصم جديد" : "Add New Discount Note")
                        }
                      </h3>
                    </div>
                  </div>
                ...
                <div className="p-4 md:p-6 max-h-[60vh] sm:max-h-[72vh] overflow-y-auto space-y-6 w-full">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                    ...
                    <div className="space-y-1.5 w-full">
                      <label className="block text-[11px] font-extrabold text-slate-500 uppercase tracking-wider">{isAr ? "قيمه الخصم" : "Discount Value"}</label>
                      <input 
                        type="number" 
                        value={discountValue} 
