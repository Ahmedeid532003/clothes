  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      <div 
        ...
        className="w-full bg-slate-50 border border-gray-300 text-slate-700 text-xs rounded-[8px] p-2.5 flex justify-between items-center cursor-pointer hover:border-orange-300 transition"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute z-50 mt-1 w-full bg-white border border-gray-300 rounded-[8px] shadow-xl overflow-hidden flex flex-col max-h-[170px]"
          >
            <div className="p-2 border-b border-gray-300 bg-slate-50/50">
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-white border border-gray-300 rounded-[8px] py-1.5 px-8 text-xs outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400"
                />
            ...
            <div className="overflow-y-auto p-1 flex-1">
              {options.length > 0 && (
                <div 
                  ...
                  className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded-[8px] cursor-pointer text-xs font-bold text-slate-700"
                >
                  <div className={cn("w-4 h-4 rounded border flex items-center justify-center transition-colors", selected.length === options.length ? "bg-orange-500 border-orange-500" : "border-gray-300 bg-white")}>
              ...
              {filteredOptions.length > 0 ? filteredOptions.map(opt => (
                <div 
                  ...
                  className="flex items-center gap-2 p-2 hover:bg-slate-50 rounded-[8px] cursor-pointer text-xs font-bold text-slate-700"
                >
                  <div className={cn("w-4 h-4 rounded border flex items-center justify-center transition-colors", selected.includes(opt) ? "bg-orange-500 border-orange-500" : "border-gray-300 bg-white")}>
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-start gap-4 flex-1 w-full mt-4">
            <div className="flex flex-col border border-[#e5ebf2] rounded-lg p-2 bg-white relative pt-3.5 w-full md:w-auto flex-none md:-me-2 md:pe-2" style={{ height: 92, minWidth: isAr ? 161 : 140 }}>
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                <label onClick={() => setModType('increase')} className={cn("flex items-center justify-between gap-2 py-1 px-2 border rounded cursor-pointer transition w-full", modType === 'increase' ? "border-orange-500 bg-orange-50/50" : "border-gray-300 bg-white hover:border-gray-300")} style={{ minWidth: isAr ? 143 : 122 }}>
                  ...
                  <div className={cn("w-3.5 h-3.5 rounded-sm flex items-center justify-center border transition-colors shrink-0", modType === 'increase' ? "bg-orange-500 border-orange-500 text-white" : "border-gray-300 bg-white")}>
                    {modType === 'increase' && <Check size={10} strokeWidth={4} />}
                  </div>
                </label>
                <label onClick={() => setModType('decrease')} className={cn("flex items-center justify-between gap-2 py-1 px-2 border rounded cursor-pointer transition w-full", modType === 'decrease' ? "border-orange-500 bg-orange-50/50" : "border-gray-300 bg-white hover:border-gray-300")} style={{ minWidth: isAr ? 143 : 122 }}>
                  ...
                  <div className={cn("w-3.5 h-3.5 rounded-sm flex items-center justify-center border transition-colors shrink-0", modType === 'decrease' ? "bg-orange-500 border-orange-500 text-white" : "border-gray-300 bg-white")}>
            ...
            <div className="relative flex flex-col gap-1 w-full md:flex-1 md:max-w-xl md:mt-[37px]">
              ...
              <div className="relative text-[#111212]">
                <input 
                  ...
                  placeholder="0.00"
                  className="w-full md:w-[237px] bg-white border border-gray-300 text-slate-800 text-sm font-black rounded-lg focus:ring-orange-500 focus:border-orange-500 block h-[36px] px-3 outline-none"
                />
