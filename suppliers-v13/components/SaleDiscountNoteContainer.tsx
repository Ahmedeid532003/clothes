...
interface SaleDiscountNoteContainerProps {
  lang: 'ar' | 'en';
...
export const SaleDiscountNoteContainer: React.FC<SaleDiscountNoteContainerProps> = ({
  lang,
  filteredSuppliers,
  groups,
  setViewingSupplier,
  ...
  const customColumnLabels: Record<string, { en: string; ar: string }> = {
    docNo: { en: 'Document No', ar: 'رقم المستند' },
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="hidden md:block bg-white border border-gray-300 rounded-xl overflow-visible shadow-sm relative pt-[13px] mt-[0px] mb-[0px] min-h-fit -mx-[8px]">
        <div className="h-[39px] px-3 border-b border-[#eaeff2] bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-3 w-full rounded-t-xl overflow-visible mt-[-13px] relative z-40">
        ...
        {dirViewMode === 'table' ? (
            ...
            <table className="w-full text-left rtl:text-right border-collapse text-xs font-bold text-slate-700">
              <thead className="sticky top-0 z-30 bg-orange-500 text-white select-none">
                <tr className="bg-[#f06424] text-white font-extrabold border-b border-orange-600">
                  {visibleColumns.docNo && <th className="p-3.5 h-[35.5px] min-w-[120px] text-center">{lang === 'ar' ? 'رقم المستند' : 'Document No'}</th>}
                  {visibleColumns.date && <th className="p-3.5 h-[35.5px] min-w-[120px] text-center">{lang === 'ar' ? 'تاريخ' : 'Date'}</th>}
                  {visibleColumns.name && <th className="p-3.5 h-[35.5px] min-w-[185px] text-start">{lang === 'ar' ? 'اسم المورد' : 'Supplier Name'}</th>}
                  {visibleColumns.season && <th className="p-3.5 h-[35.5px] text-center">{lang === 'ar' ? 'الموسم' : 'Season'}</th>}
                  {visibleColumns.discountValue && <th className="p-3.5 h-[35.5px] text-center">{lang === 'ar' ? 'قيمة الخصم' : 'Discount Value'}</th>}
                  ...
                  {isColumnFiltersOpen && (
                    <tr className="bg-slate-100 border-b border-gray-300 text-slate-700">
                      {visibleColumns.docNo && <td className="py-2 px-3"></td>}
                      {visibleColumns.date && <td className="py-2 px-3"></td>}
                      {visibleColumns.name && (
                      ...
                      {visibleColumns.season && <td className="py-2 px-3"></td>}
                      {visibleColumns.discountValue && <td className="py-2 px-3"></td>}
                    ...
                    paginatedSuppliers.map((s) => (
                      <tr key={s.id} className="hover:bg-slate-50 transition-colors border-b border-gray-300">
                        {visibleColumns.docNo && (
                          <td className="p-3 text-center font-mono text-[11px] text-slate-600">{s.id || '—'}</td>
                        )}
                        {visibleColumns.date && (
                          <td className="p-3 text-center font-mono text-[11px] text-slate-600">{s.inventoryDay || '—'}</td>
                        )}
                        {visibleColumns.name && (
                          <td className="p-3">
                        ...
                        {visibleColumns.discountValue && (
                          <td className="p-3 text-center font-mono text-[11px] text-slate-600">{s.checkName || '—'}</td>
                        )}
                        <td className="p-3 text-center">
              ...
              {paginatedSuppliers.map((s) => (
                <div key={s.id} className="bg-white border border-gray-300 rounded-xl p-5 shadow-xs flex flex-col justify-between hover:border-orange-300 transition-all text-right">
                  <div className="flex justify-between items-center mb-3.5 rtl:flex-row-reverse">
                    <span className="text-[9.5px] font-black px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-gray-300">
                      {lang === 'ar' ? 'رقم المستند: ' : 'Doc No: '} {s.id || '—'}
                    </span>
                    <span className="px-2 py-0.5 bg-orange-50 text-orange-600 text-[9px] font-black rounded border border-orange-100 uppercase">
                      {s.type || '—'}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 bg-slate-50 p-2.5 rounded-xl border border-gray-300 mb-3 text-[10px] font-bold">
                    <div>
                      <span className="text-slate-400 block text-[8px] uppercase tracking-wider">{lang === 'ar' ? 'التاريخ' : 'Date'}</span>
                      <span className="font-mono text-slate-700">{s.inventoryDay || '—'}</span>
                    </div>
                    <div className="border-r border-gray-300 pr-2">
                      <span className="text-emerald-400 block text-[8px] uppercase tracking-wider">{lang === 'ar' ? 'قيمة الخصم' : 'Discount'}</span>
                      <span className="font-mono text-emerald-600">{s.checkName || '—'}</span>
                    </div>
                  </div>
          ...
          {/* Desktop Pagination */}
          <div className="px-4 h-[41px] border-t border-black border-x border-white border-b border-[#141313] flex flex-col md:flex-row items-center justify-between gap-4 text-xs bg-slate-50/40 rounded-b-xl text-slate-600">
