...
import { Search, Trash2, Edit, X, Plus, History, ChevronDown, ArrowLeft, ArrowRight , Eye} from 'lucide-react';
...
export interface PurchaseInvoiceCreatorProps {
  lang: 'ar' | 'en';
  invoiceData: any;
  onCancel: () => void;
...
export function PurchaseInvoiceCreator({ lang, invoiceData, onCancel, onSave, products }: PurchaseInvoiceCreatorProps) {
  const isAr = lang === 'ar';
  
  // Sidebar states
  const [invoiceDate, setInvoiceDate] = useState(invoiceData?.invoiceDate || new Date().toISOString().split('T')[0]);
  const [supplierName, setSupplierName] = useState(invoiceData?.supplierName || "");
  const [season, setSeason] = useState(invoiceData?.season || "");
  const [brand, setBrand] = useState(invoiceData?.brand || "");
  const [itemGroup, setItemGroup] = useState(invoiceData?.itemGroup || "");
  
  // New Header Fields
  const [category, setCategory] = useState("");
  const [branchStore, setBranchStore] = useState("");
  const [invoiceType, setInvoiceType] = useState("اجل");
  ...
  useEffect(() => {
    if ((invoiceType === 'نقدى واجل' || invoiceType === 'متنوع') && paidAmountInputRef.current) {
  ...
  const [globalProfitPercent, setGlobalProfitPercent] = useState<number>(40);
  const [searchBySeasonOnly, setSearchBySeasonOnly] = useState<boolean>(false);
  const [searchBySupplierOnly, setSearchBySupplierOnly] = useState<boolean>(false);
  ...
  const [editSellPrice, setEditSellPrice] = useState<number | "">("");
  ...
  const handleQtyKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && selectedProduct && qtyInput) {
      if (!supplierName || !branchStore || !season || !category) {
        setValidationError(isAr ? "برجاء اختيار المورد، الفرع، الموسم، والقسم أولاً" : "Please select Supplier, Branch, Season, and Category first");
        return;
      }
      setValidationError("");
      ...
      const newItem = {
        ...
        profitPercent: globalProfitPercent,
        sellPrice: buyPrice * (1 + globalProfitPercent / 100),
        repeat: 1,
  ...
  const saveEditedItem = () => {
    if (editingIndex !== null) {
      ...
      if (editSellPrice !== "") item.sellPrice = Number(editSellPrice);
  ...
  const invoiceTypeOptions = [
    ...
    { value: "نقدى واجل", label: isAr ? "نقدى واجل" : "Cash & Credit" },
    { value: "شيك", label: isAr ? "شيك" : "Check" },
    { value: "متنوع", label: isAr ? "متنوع" : "Misc" }
  ];
    ...
    <div className={cn("fixed inset-0 z-[200] w-full h-full flex flex-col bg-slate-50 lg:bg-gray-100 overflow-hidden", isAr ? "rtl font-[Cairo]" : "ltr")}>
      ...
      <div className="flex lg:hidden flex-col h-full bg-slate-50 relative pb-[60px]">
         ...
         <div className="bg-[#FF6900] text-white px-4 py-2 shrink-0 flex items-center justify-between shadow-md h-[43px]">
            <span className="font-bold text-[14px]">{isAr ? "انشاء فاتوره مشتريات" : "Create Purchase Invoice"}</span>
         ...
         <div className="flex-1 overflow-y-auto p-2 space-y-3">
            <div className="bg-white p-3 rounded-[12px] shadow-sm border border-gray-200 flex flex-col gap-3">
               <div className="space-y-2">
                 <input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)} className="w-full border border-gray-300 rounded-[8px] px-2 h-[34px] outline-none focus:border-[#FF6900] text-[11px] font-bold text-gray-700 bg-white" />
                 ...
                 <div className="w-full h-[34px]"><CustomSelect value={brand} onChange={setBrand} options={brandOptions} placeholder={isAr ? "البراند" : "Brand"} isAr={isAr} /></div>
                 ...
                 <div className="w-full h-[34px]"><CustomSelect value={category} onChange={setCategory} options={categoryOptions} placeholder={isAr ? "القسم" : "Category"} isAr={isAr} /></div>
                 <div className="w-full h-[34px]"><CustomSelect value={itemGroup} onChange={setItemGroup} options={groupOptions} placeholder={isAr ? "المجموعة" : "Group"} isAr={isAr} /></div>
               </div>
               <div className="space-y-2 border-t border-gray-100 pt-3">
                  <div className="w-full h-[34px]">
                    <CustomSelect value={invoiceType} onChange={setInvoiceType} options={invoiceTypeOptions} placeholder={isAr ? "نوع الفاتوره" : "Invoice Type"} isAr={isAr} />
                  </div>
                  {invoiceType === 'نقدى واجل' && (
                    <div className="h-[34px]">
                      <input 
                        ref={paidAmountInputRef}
                        type="number" 
                        placeholder={isAr ? "مدفوع 0" : "Paid 0"}
                        className="w-full h-full border border-gray-300 rounded-[8px] px-2 outline-none focus:border-[#FF6900] text-[12px] font-bold text-center text-gray-700"
                      />
                    </div>
                  )}
                  {invoiceType === 'شيك' && (
                    <div className="h-[34px] flex gap-1">
                      <div className="flex-1 h-full">
                        <CustomSelect value={checkNumber} onChange={setCheckNumber} options={checkOptions} placeholder={isAr ? "رقم الشيك" : "Check Number"} isAr={isAr} />
                      </div>
                      <button className="h-full w-[34px] shrink-0 bg-[#FF6900] text-white rounded-[8px] flex items-center justify-center hover:bg-[#e65c00] transition-colors">
                        <Plus size={16} />
                      </button>
                    </div>
                  )}
              ...
              <div className="flex gap-2">
                 <div className="flex-1 flex items-center h-[34px] border border-gray-300 rounded-[8px] overflow-hidden bg-white hover:border-[#FF6900] transition-colors focus-within:border-[#FF6900] shrink-0">
                    <div className="bg-gray-100 px-2 h-full flex items-center justify-center font-bold text-[11px] border-l border-gray-300 shrink-0 text-gray-700">
                       {isAr ? "هامش ربح %" : "Profit %"}
                    </div>
                    <input 
                       type="number" 
                       value={globalProfitPercent}
                       onChange={e => setGlobalProfitPercent(Number(e.target.value))}
                       className="w-full h-full outline-none text-center text-[10px] font-bold"
                    />
                 </div>
              </div>
              <div className="flex gap-2">
                 ...
                 <div className="flex-1 flex items-center h-[34px] border border-gray-300 rounded-[8px] overflow-hidden bg-white hover:border-[#FF6900] transition-colors focus-within:border-[#FF6900] shrink-0">
                    <div className="bg-gray-100 px-2 h-full flex flex-1 items-center justify-center font-bold text-[10px] font-[Cairo] border-l border-gray-300 shrink-0 text-gray-700">
                       {isAr ? "منتجات المورد فقط" : "Supplier only"}
                    </div>
                    <div className="w-[30px] h-full flex items-center justify-center">
                       <input 
                           type="checkbox" 
                           checked={searchBySupplierOnly}
                          onChange={e => setSearchBySupplierOnly(e.target.checked)}
                          className="w-3.5 h-3.5 cursor-pointer accent-[#FF6900]"
                       />
                    </div>
                 </div>
              </div>
              <div className="flex-1 relative h-[45px] border rounded-[8px] bg-gray-50 flex items-center">
                       ...
                       <div className="flex flex-col">
                          ...
                          <table className="w-full text-[11px] font-bold text-center">
                            <thead className="bg-gray-100 sticky top-0 border-b border-gray-300 shadow-sm">
                              <tr>
                                ...
                                <th className="p-2 border-l border-gray-300">{isAr ? "البراند" : "Brand"}</th>
                            ...
                            <tbody>
                              {filteredProducts.map((p, idx) => (
                                ...
                                >
                                  ...
                                  <td className="p-2 border-l border-gray-200">{p.brand || "-"}</td>
                        ...
                        <div 
                           ...
                           onClick={() => {
                             ...
                             setIsAddProductModalOpen(true); setIsSearchOpen(false);
                           }}
                         >
      ...
      <div className="hidden lg:flex flex-col flex-1 overflow-hidden bg-white">
        ...
        <div className="bg-[#FF6900] text-white px-4 shrink-0 flex items-center justify-between shadow-md h-[43px] z-30">
          <div className="font-bold text-[14px]">{isAr ? "انشاء فاتوره مشتريات" : "Create Purchase Invoice"}</div>
          <button 
            onClick={onCancel}
        ...
        <div className="bg-white px-4 py-2 border-b border-gray-300 shrink-0 z-20 flex justify-between">
          <div className="flex gap-2">
            <div className="flex flex-col gap-2">
              <div className="flex gap-2 h-[34px]">
                ...
                <div className="w-[140px] h-full"><CustomSelect value={brand} onChange={setBrand} options={brandOptions} placeholder={isAr ? "البراند" : "Brand"} isAr={isAr} /></div>
              </div>
              <div className="flex gap-2 h-[34px]">
                ...
                <div className="w-[140px] h-full"><CustomSelect value={category} onChange={setCategory} options={categoryOptions} placeholder={isAr ? "القسم" : "Category"} isAr={isAr} /></div>
                <div className="w-[140px] h-full"><CustomSelect value={itemGroup} onChange={setItemGroup} options={groupOptions} placeholder={isAr ? "المجموعة" : "Group"} isAr={isAr} /></div>
              </div>
            </div>
            
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 h-[34px]">
                <span className="font-bold text-[11px] text-gray-700 whitespace-nowrap">{isAr ? "نوع الفاتوره" : "Invoice Type"}</span>
              ...
              {invoiceType === 'نقدى واجل' && (
                <div className="h-[34px] flex items-center justify-end">
                  <input 
                    ...
                    type="number" 
                    placeholder={isAr ? "مدفوع 0" : "Paid 0"}
                    className="w-[180px] ml-0 mr-[66px] h-full border border-gray-300 rounded-[8px] px-2 outline-none focus:border-[#FF6900] text-[12px] font-bold text-center text-gray-700"
                  />
                </div>
              )}
              {invoiceType === 'شيك' && (
                <div className="h-[34px] flex items-center justify-end">
                  <div className="w-[180px] h-full flex gap-1">
                    <div className="flex-1 h-full">
                      <CustomSelect value={checkNumber} onChange={setCheckNumber} options={checkOptions} placeholder={isAr ? "رقم الشيك" : "Check Number"} isAr={isAr} />
                    </div>
                    <button className="h-full w-[34px] shrink-0 bg-[#FF6900] text-white rounded-[8px] flex items-center justify-center hover:bg-[#e65c00] transition-colors">
              ...
              {invoiceType === 'متنوع' && (
                <div className="h-[34px] flex items-center justify-end gap-2 -ml-[7px]">
                  <input 
                    ref={paidAmountInputRef}
                    type="number" 
                    placeholder={isAr ? "مدفوع 0" : "Paid 0"}
                    className="w-[140px] mr-[33px] h-full border border-gray-300 rounded-[8px] px-2 outline-none focus:border-[#FF6900] text-[12px] font-bold text-center text-gray-700"
                  />
                  <div className="w-[140px] ml-0 mr-[33px] h-full flex gap-1">
                    <div className="flex-1 h-full">
                      <CustomSelect value={checkNumber} onChange={setCheckNumber} options={checkOptions} placeholder={isAr ? "رقم الشيك" : "Check Number"} isAr={isAr} />
                    </div>
                    <button className="h-full w-[34px] shrink-0 bg-[#FF6900] text-white rounded-[8px] flex items-center justify-center hover:bg-[#e65c00] transition-colors">
          ...
          <div className="flex flex-col gap-2 w-[211px] ml-0 pl-0 pr-[25px] h-[76px] -mt-[11px] mb-0 pt-[7px]">
            <div className="flex gap-[5px] w-[200px] ml-0 -mr-[18px] -mb-[1px]">
              <input type="date" value={invoiceDate} onChange={e => setInvoiceDate(e.target.value)} className="w-[110px] h-[30px] border border-gray-300 rounded-[7px] px-2 outline-none focus:border-[#FF6900] text-[11px] font-bold text-gray-700 bg-white" />
              <button 
                onClick={() => setIsOrderModalOpen(true)}
                className="w-[85px] h-[30px] bg-[#FF6900] text-white rounded-[7px] flex items-center justify-center gap-1 hover:bg-[#e65c00] transition-colors font-bold text-[11px]"
              >
                <Plus size={14} />
                <span>{isAr ? "من اوردر" : "From Order"}</span>
              </button>
            </div>
            <div className="w-[200px] ml-0 -mr-[18px] -mt-[4px] h-[30px] flex items-center justify-start px-2 font-bold text-[11px] text-gray-600 bg-gray-50 rounded-[8px] border border-gray-200">
        ...
        <div className="flex-1 flex flex-col bg-gray-50 overflow-hidden relative z-10">
          ...
          <div className="p-3 shrink-0 flex gap-3 h-[58px] items-center border-b border-gray-300 bg-white">
             ...
             <div className="flex items-center h-[34px] border border-gray-300 rounded-[8px] overflow-hidden bg-white hover:border-[#FF6900] transition-colors focus-within:border-[#FF6900] shrink-0">
                ...
                <input 
                   ...
                   value={globalProfitPercent} 
                   onChange={e => setGlobalProfitPercent(Number(e.target.value))}
                   className="w-[50px] h-full outline-none text-center text-[10px] font-bold"
                />
             </div>
             ...
             <div className="flex items-center h-[34px] border border-gray-300 rounded-[8px] overflow-hidden bg-white hover:border-[#FF6900] transition-colors focus-within:border-[#FF6900] shrink-0">
                <div className="bg-gray-100 px-2 h-full flex items-center justify-center font-bold text-[10px] font-[Cairo] border-l border-gray-300 shrink-0 text-gray-700">
                   {isAr ? "بحث بمنتجات المورد فقط" : "Search by supplier only"}
                </div>
                <div className="w-[30px] h-full flex items-center justify-center">
                   <input 
                      type="checkbox" 
             ...
             {/* Search */}
             <div className="flex-1 relative h-[34px] border border-gray-300 rounded-[8px] bg-white hover:border-[#FF6900] transition-colors focus-within:border-[#FF6900]">
            ...
            <table className="w-full text-center text-[11px] font-bold bg-white border border-gray-300 shadow-sm rounded-lg overflow-hidden">
               <thead className="bg-[#f8f9fa] border-b border-gray-300 text-gray-700">
                  <tr>
                     ...
                     <th className="p-2 border-l border-gray-300">{isAr ? "اسم الصنف" : "Item Name"}</th>
                     <th className="p-2 border-l border-gray-300 w-[100px]">{isAr ? "البراند" : "Brand"}</th>
                     ...
                     <th className="p-2 border-l border-gray-300 w-[50px]">{isAr ? "نسبة الربح" : "Profit %"}</th>
                     <th className="p-2 border-l border-gray-300 w-[70px]">{isAr ? "سعر البيع" : "Sell Price"}</th>
                     <th className="p-2 border-l border-gray-300 w-[40px]">{isAr ? "تكرار" : "Repeat"}</th>
                     <th className="p-2 border-l border-gray-300 w-[70px]">{isAr ? "سعر بيع سابق" : "Prev Sell"}</th>
                     <th className="p-2 border-l border-gray-300 w-[55px]">{isAr ? "رصيد بالفروع" : "Branches Bal."}</th>
               ...
               <tbody>
                  {items.map((item, idx) => (
                     <tr key={idx} className="border-b border-gray-200 hover:bg-orange-50 transition-colors">
                        ...
                        <td className="p-2 border-l border-gray-200">{item.brand || "-"}</td>
                        ...
                        <td className="p-2 border-l border-gray-200">{item.profitPercent}%</td>
                        <td className="p-2 border-l border-gray-200">{item.sellPrice.toLocaleString()}</td>
                        <td className="p-2 border-l border-gray-200">{item.repeat || 1}</td>
          ...
          <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            ...
            >
              ...
              >
                ...
                <div className="flex-1 space-y-2 -mt-[7px]">
                  <label className="font-bold text-[12px] text-gray-700">{isAr ? "سعر البيع" : "Sell Price"}</label>
                  <input 
                    type="number" 
                    value={editSellPrice} 
                    onChange={e => setEditSellPrice(e.target.value === "" ? "" : Number(e.target.value))}
                    className="w-full border-2 border-gray-300 rounded-[8px] h-[34px] outline-none focus:border-[#FF6900] font-bold text-center text-emerald-600 text-[12px]"
                  />
                </div>
              </div>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="bg-[#FF6900] px-4 flex justify-between items-center text-white h-[56px] rounded-t-[12px] shrink-0">
                <h3 className="font-bold text-[15px]">{isAr ? "ادخل رقم الاوردر" : "Enter Order Number"}</h3>
              ...
              <div className="p-4 flex flex-col gap-4 flex-1 overflow-hidden">
                <div className="relative shrink-0">
                  ...
                  <input 
                    ...
                    placeholder={isAr ? "بحث برقم الاوردر..." : "Search order number..."}
                ...
                <div className="border border-gray-200 rounded-[8px] overflow-auto flex-1 bg-white">
                  <table className="w-full text-sm text-right">
                    <thead className="bg-gray-50 border-b border-gray-200 text-[12px] text-gray-600 sticky top-0">
                      <tr>
                        <th className="px-3 py-2.5 font-bold">{isAr ? "رقم الاوردر" : "Order No."}</th>
                        <th className="px-3 py-2.5 font-bold">{isAr ? "قيمة الاوردر" : "Order Value"}</th>
                        <th className="px-3 py-2.5 font-bold">{isAr ? "المستخدم" : "User"}</th>
                        <th className="px-3 py-2.5 font-bold">{isAr ? "تاريخ الاوردر" : "Order Date"}</th>
                        <th className="px-3 py-2.5 font-bold">{isAr ? "الفرع" : "Branch"}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        { id: "ORD-20260716-01", value: 15400, user: "محمد أحمد", date: "2026-07-16", branch: "الفرع الرئيسي", items: [{ productId: "PROD-1", barcode: "12345", description: "تيشيرت صيفي", qty: 10, buyPrice: 1500 }] },
                        { id: "ORD-20260715-02", value: 8500, user: "هاني كاشير", date: "2026-07-15", branch: "مخزن 1", items: [{ productId: "PROD-2", barcode: "67890", description: "بنطلون جينز", qty: 5, buyPrice: 1700 }] },
                        { id: "ORD-20260714-03", value: 3200, user: "محمود", date: "2026-07-14", branch: "الفرع الرئيسي", items: [{ productId: "PROD-3", barcode: "11122", description: "قميص كلاسيك", qty: 4, buyPrice: 800 }] },
                        ...
                        <tr 
                          ...
                          onClick={() => {
                             ...
                             const newItems = order.items.map(item => ({
                               ...
                               profitPercent: globalProfitPercent,
                               sellPrice: item.buyPrice * (1 + globalProfitPercent / 100),
                               repeat: 1,
                        ...
                        >
                          ...
                          <td className="px-3 py-3 font-bold text-gray-700">{order.user}</td>
      ...
      <AddProductModal 
        ...
        initialData={{
          ...
          profitMargin: globalProfitPercent
        }}
        onSave={(newProduct: any) => {
          const buyPrice = newProduct.costPrice || 0;
          const qty = Number(newProduct.quantity) || 1;
          const newItem = {
            productId: newProduct.id,
            barcode: newProduct.barcode || newProduct.code,
            model: newProduct.code || "",
            description: newProduct.nameAr || newProduct.nameEn,
            brand: brand || "",
            qty: qty,
            buyPrice: buyPrice,
            discountType: "val",
            discountValue: 0,
            total: buyPrice * qty,
            profitPercent: newProduct.profitMargin || globalProfitPercent,
            sellPrice: newProduct.sellPrice || (buyPrice * (1 + (newProduct.profitMargin || globalProfitPercent) / 100)),
            repeat: 1,
            prevSellPrice: buyPrice
          };
          setItems([newItem, ...items]);
        }}
        lang={isAr ? 'ar' : 'en'}
