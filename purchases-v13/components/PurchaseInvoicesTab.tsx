...
import { PurchaseInvoiceCreator } from './PurchaseInvoiceCreator';
...
interface PurchaseInvoicesTabProps {
  ...
  setPurchaseInvoices: (items: PurchaseInvoice[]) => void;
  groups?: any[];
}

export function PurchaseInvoicesTab({ lang, products, purchaseInvoices, setPurchaseInvoices, groups = [] }: PurchaseInvoicesTabProps) {
  ...
  const filteredProductsList = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      const pCode = (p.code || p.invoiceNo || "").toLowerCase();
      const pBarcode = (p.barcode || p.invoiceDate || "").toLowerCase();
  ...
  const mappedCompositeItems = purchaseInvoices.map(item => ({
  ...item,
  ...
  const handleUpdateComposite = () => {
    ...
    setPurchaseInvoices(purchaseInvoices.map(item => 
      item.id === editingId 
  ...
  const handleDeleteComposite = (id: string) => {
    if (confirm(isAr ? "هل أنت متأكد من الحذف؟" : "Are you sure you want to delete?")) {
      setPurchaseInvoices(purchaseInvoices.filter(item => item.id !== id));
    }
  };
  ...
  const uniqueAmounts = Array.from(new Set(purchaseInvoices.map(inv => inv.totalAmount))).map(a => ({ id: a?.toString() || "", nameEn: a?.toString() || "", nameAr: a?.toString() || "" }));
  const uniquePaymentMethods = Array.from(new Set(purchaseInvoices.map(inv => inv.paymentMethod).filter(Boolean))).map(p => ({ id: p, nameEn: p, nameAr: p }));
  const uniqueUsers = Array.from(new Set(purchaseInvoices.map(inv => inv.username).filter(Boolean))).map(u => ({ id: u, nameEn: u, nameAr: u }));
  const uniqueBranches = Array.from(new Set(purchaseInvoices.map(inv => inv.branch).filter(Boolean))).map(b => ({ id: b, nameEn: b, nameAr: b }));
        ...
        <div className="flex items-center justify-between h-full gap-4">
          <div className="space-y-0.5 overflow-hidden">
            <h2 className="text-base sm:text-lg font-bold text-slate-800 flex items-center gap-2 truncate">
              ...
              <span className="truncate">{isAr ? "فواتير المشتريات" : "Purchase Invoices"}</span>
            </h2>
            <p className="text-[10px] sm:text-xs text-slate-500 truncate">
              {isAr ? "إدارة وعرض جميع فواتير المشتريات" : "Manage and view all purchase invoices"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              ...
              <span className="text-[11px] whitespace-nowrap font-black hidden sm:inline-block">
                {isAr ? "فاتورة مشتريات جديدة" : "New Invoice"}
              </span>
            </button>
          </div>
        </div>
        ...
        <div className="mt-8">
          <ProductsListContainer
            ...
            initialViewMode={window.innerWidth <= 1024 ? "kanban" : "table"}
                                                customColumnLabels={{
              code: { en: 'Invoice Number', ar: 'رقم الفاتوره' },
              barcode: { en: 'Invoice Date', ar: 'تاريخ الفاتوره' },
              name: { en: 'Supplier Name', ar: 'اسم المورد' },
              brand: { en: 'Invoice Value', ar: 'قيمه الفاتوره' },
