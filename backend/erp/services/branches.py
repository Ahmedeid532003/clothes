...
def is_sale_outlet_warehouse(warehouse_id) -> bool:
    return BranchWarehouse.objects.using("tenant").filter(warehouse_id=warehouse_id).exists()
...
def ensure_branch_sale_warehouse(branch: Branch) -> Warehouse:
    ...
    if link and link.warehouse.is_active:
        ...
        if wh.primary_branch_id != branch.id:
            ...
            wh.save(using="tenant", update_fields=["primary_branch", "updated_at"])
        return wh
    ...
    BranchWarehouse.objects.using("tenant").filter(branch=branch).exclude(
        warehouse=wh
    ).update(is_default=False)
    BranchWarehouse.objects.using("tenant").update_or_create(
        branch=branch,
        warehouse=wh,
...
def sync_all_branch_sale_warehouses() -> int:
    count = 0
    for branch in Branch.objects.using("tenant").filter(is_active=True):
